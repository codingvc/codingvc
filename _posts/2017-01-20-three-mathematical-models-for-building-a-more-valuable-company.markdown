...
## The value of models
This post presented a few (somewhat arbitrary) mental models for valuing startups. None of the models is ideal**, but each of them has interesting implications for how to maximize the long-term value of a company. While no early stage investor or founder should be calculating their startup's value using these formulas, the implications are still relevant: you should mostly focus on near term milestones and not things you won't need for several years, you should beef up your company's biggest weaknesses, you should prioritize building a strong team because that has a large impact on everything else in the company, and so on.
...
3. In the third model, there are many milestones that can be done in parallel or that do not depend on each other. For example, you can build a sales team and explore marketing channels at the same time, and making progress on either goal will provide your company's valuation.